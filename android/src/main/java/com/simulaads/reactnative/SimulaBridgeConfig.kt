...
internal fun ReadableMap?.toSimulaApiEnvironment(): SimulaApiEnvironment = runCatching {
    val map = this
    if (map != null && map.hasKey("apiEnvironment") && !map.isNull("apiEnvironment") &&
        map.getType("apiEnvironment") == ReadableType.String && map.getString("apiEnvironment") == "staging"
    ) SimulaApiEnvironment.Staging else SimulaApiEnvironment.Production
